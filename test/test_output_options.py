...
def test_transcribe_output_directory() -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        logging.info(f"✓ Basic output directory test passed - file created: {expected_txt}")
        
        # Test with --complex-json option
        cmd_json = cmd + ["--complex-json"]
        logging.info("Testing output directory with --complex-json...")
        
        result = subprocess.run(cmd_json, capture_output=True, text=True, cwd=Path.cwd().parent)
        if result.returncode != 0:
        ...
        if not expected_json.exists():
            raise AssertionError(f"Expected JSON output file not found: {expected_json}")
        ...
        logging.info(f"✓ Output directory with JSON test passed - files created: {expected_txt}, {expected_json}")
        ...
        cmd_nested = [
            "python", "transcribe.py",
            str(test_audio_file.absolute()),
            "--out-dir", str(nested_dir),
            "--force"
        ]
        
        logging.info("Testing nested directory creation...")
        result = subprocess.run(cmd_nested, capture_output=True, text=True, cwd=Path.cwd().parent)
        
        if result.returncode != 0:
            raise RuntimeError(f"Nested directory transcription failed: {result.stderr}")
        
        expected_nested_txt = nested_dir / f"{test_audio_file.stem}.txt"
        if not expected_nested_txt.exists():
            raise AssertionError(f"Expected nested output file not found: {expected_nested_txt}")
        
        logging.info(f"✓ Nested directory creation test passed - file created: {expected_nested_txt}")
...
def test_post_process_output_directory() -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if processed_content.strip() == test_content.strip():
            logging.warning("Processed content appears identical to original (this may be normal for test mode)")
        
        logging.info(f"✓ Post-process output directory test passed - file created: {expected_output}")
...
def test_post_process_extension_suffix() -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if result.returncode != 0:
            raise RuntimeError(f"Post-processing with extension failed: {result.stderr}")
        ...
        if not expected_output.exists():
            raise AssertionError(f"Expected output file with extension not found: {expected_output}")
        
        logging.info(f"✓ Extension suffix test passed - file created: {expected_output}")
        ...
        result = subprocess.run(cmd_combined, capture_output=True, text=True, cwd=Path.cwd().parent)
        ...
        if not expected_combined.exists():
            raise AssertionError(f"Expected combined output file not found: {expected_combined}")
        
        logging.info(f"✓ Combined output directory and extension test passed - file created: {expected_combined}")
...
def test_post_process_inplace() -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        result = subprocess.run(cmd_verify, capture_output=True, text=True, cwd=Path.cwd().parent)
        
        if result.returncode != 0:
            logging.warning(f"In-place with verification may have failed due to test mode: {result.stderr}")
        else:
...
def test_error_conditions() -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        cmd = [
            "python", "transcribe.py",
            str(test_audio_file.absolute()),
            "--out-dir", str(non_existent_parent),
            "--force"
        ]
        ...
        cmd_conflict = [
            ...
            str(test_file.absolute()),
            "--inplace",
            "--output", str(temp_path / "output.txt")
        ]
        
        logging.info("Testing conflicting options (should fail)...")
        result = subprocess.run(cmd_conflict, capture_output=True, text=True, cwd=Path.cwd().parent)
        
        # Should fail due to mutually exclusive options
        if result.returncode == 0:
            logging.warning("Expected failure for conflicting options, but command succeeded")
        else:
