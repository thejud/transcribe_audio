...
"""
Post-Processing Tool for Transcript Formatting

This script takes a text transcript file and uses OpenAI's chat models to 
reformat the text into natural paragraphs while preserving all original
text and wording exactly.

Features:
- Support for gpt-4o and gpt-4o-mini models
- Preserves original text while improving readability
...
Usage:
    ...
    python post_process.py transcript.txt --mini
...
def get_chunk_cache_key(chunk_text: str, model: str, context: str = "") -> str:
    """
    ...
    Returns:
        str: MD5 hash of the chunk + model + context
    """
    content = f"{chunk_text}|{model}|{context}"
...
def split_into_chunks(
    ...
    logging.info(f"Split text into {len(chunks)} chunks (avg {current_tokens//len(chunks) if chunks else 0} tokens/chunk)")
    return chunks
...
async def reformat_chunk_async(
    ...
    """
    system_prompt = (
        "Please reformat this transcript chunk into natural paragraphs while preserving "
...
async def reformat_transcript_chunked(
    ...
    try:
        ...
        if total_tokens <= chunk_size:
            ...
            response = await reformat_chunk_async(transcript, client, model, "", 0)
            return response
        
        # Split into chunks
        chunks = split_into_chunks(transcript, chunk_size, overlap, model)
        logging.info(f"Processing {len(chunks)} chunks with max {max_concurrent} concurrent requests")
        ...
        async def process_chunk_with_semaphore(chunk_info: Tuple[str, int, int], idx: int) -> Tuple[int, str]:
            ...
            cache_key = None
            if cache_path:
                cache_key = get_chunk_cache_key(chunk_text, model, context)
            ...
            async with semaphore:
                ...
                reformatted = await reformat_chunk_async(chunk_text, client, model, context, idx)
        ...
        for i in range(1, len(reformatted_chunks)):
            ...
            prev_sentences = final_text.split(". ")[-3:]  # Last 3 sentences from previous
            current_sentences = current_chunk.split(". ")[:3]  # First 3 sentences from current
            
            # Find overlap and remove it
            overlap_found = False
            for j in range(len(prev_sentences)):
                for k in range(len(current_sentences)):
                    if prev_sentences[j].strip() and prev_sentences[j] in current_sentences[k]:
...
def verify_text_integrity(original_text: str, processed_text: str, mode: str = "normalized") -> bool:
    """
    ...
    if mode == "strict":
        ...
        return normalize_whitespace_for_verification(original_text) == normalize_whitespace_for_verification(processed_text)
    elif mode == "word-only":
...
def reformat_transcript(transcript: str, client: OpenAI, model: str = "gpt-4.1-nano") -> str:
    """
    ...
    """
    prompt = (
        "Please reformat this transcript into natural paragraphs while preserving "
        "all original text and wording exactly. Only add paragraph breaks where they "
        "improve readability. Do not change, add, or remove any words - just improve "
        "the paragraph structure."
    )
...
Examples:
  ...
  %(prog)s transcript.txt --debug                  # Enable debug logging
    ...
    parser.add_argument(
        "transcript_file",
        help="Path to the transcript file to reformat"
    )
...
async def main_async() -> None:
    ...
    if args.test_mode:
        ...
        logging.info(f"Test mode: Processing {len(transcript_text)} characters (reduced from {original_length})")
    ...
    if args.no_chunking:
        ...
        reformatted_text = reformat_transcript(transcript_text, sync_client, args.model)
    else:
    ...
    if args.verify:
        ...
        integrity_check = verify_text_integrity(transcript_text, reformatted_text, args.verify_mode)
        ...
        if integrity_check:
            ...
            print("The processed text does not preserve the original content according to the verification criteria.", file=sys.stderr)
